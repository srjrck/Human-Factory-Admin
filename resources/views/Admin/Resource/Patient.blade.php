              ...
              <div class="row">
                ...
                <div class="col-sm-6">
                  <a href="{{route('AddPatient')}}">
                    <button type="button" class="btn btn-secondary m-1 pull-right">Add Patient</button>
                  </a>
                </div>
              </div>
                ...
                <table class="table table-bordered" id="default-datatable">
                  ...
                  <tbody>
                    ...
                    foreach($Result as $row){
                      ...
                      //echo '<pre>';print_r($Response->contained[0]->telecom[0]->value);die;
                      //echo "<pre>";print_r(json_decode($row->json['contained'][0], true));
                    ?>  
                      <tr>
                        <td>{{$i}}</td>
                       <!-- <td>{{$identifier->value}}</td>-->
                        <td>{{$name->family}}</td>
                        <td>{{$Response->address[0]->line}}</td>
                        <td>{{$Response->birthDate}}</td>
                        <td>{{$Response->address[0]->postalCode}}</td>
                        <td>{{$Response->contained[0]->telecom[0]->value}}</td>
                        <td>{{$Response->contained[0]->telecom[1]->value}}</td>
                        <td>
                          <a href="<?=route('ViewPatient',array('ID'=>base64_encode($row->id)))?>">
                            <button class="btn btn-success"> <i class="fa fa-eye"></i> </button>
                          </a>
                          <a href="<?=route('EditPatient',array('ID'=>base64_encode($row->id)))?>">
                            <button class="btn btn-primary"> <i class="fa fa-edit"></i> </button>
                          </a>
                          <a href="<?=route('DeletePatient',array('ID'=>base64_encode($row->id)))?>" onclick="return confirm('Are you sure?')">
